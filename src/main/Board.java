...
public class Board extends JPanel {
	...
	private Tile lastJumpTile;
	private Boolean isNJumping = false;
			...
			if(t.HasPiece()) {
				...
				if(!color.equals(tempPiece.getColor())) {
					...
					if(tempTile != null && !tempTile.HasPiece()) {
						tempTile.addJumpingPiece(t.getPiece());
						if(isNJumping && this.lastJumpTile != null) {
							for(Piece p:this.lastJumpTile.getJumpingPiece())
								tempTile.addJumpingPiece(p);
						}
...
//			System.out.println("for loop12");
		...
		this.isNJumping = false;
		this.lastJumpTile = null;
	}
...
//				System.out.println("for loop14");
		...
		if(validTarget) {
			...
			if(getTileAt(target.y,target.x).isJumpTile()) {
				for(Piece p:getTileAt(target.y,target.x).getJumpingPiece())
...
//				System.out.println("for loop16");
				t.setJumpTile(false);
				t.setJumpingPiece(new ArrayList<Piece>());
			}
